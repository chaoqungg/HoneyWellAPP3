...
import com.honeywell.honeywellproject.BleSuperModule.widge.BleSuperActivity;
...
public class LoginActivity extends statusActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            getWindow().setStatusBarColor(getResources().getColor(R.color.gray_shadow));
        }
    }
    ...
    protected void onResume() {
        super.onResume();
        int currentusertype = SharePreferenceUtil.getIntSP("currentusertype");
        String currentusername = SharePreferenceUtil.getStringSP("currentusername","");
        ...
        logininfolist= DataSupport.findAll(LoginInfoBean.class);
                    ...
                    if(et.getId()==R.id.et_username){
                        if(logininfolist!=null && logininfolist.size()>0){
                            for(int k=0;k<logininfolist.size();k++){
                                LoginInfoBean bean = logininfolist.get(k);
    ...
    @OnClick({R.id.btn_login, R.id.iv_usertype_arrow,R.id.btn_register,
            R.id.iv_username_arrow, R.id.iv_password_eye,R.id.btn_modifypassword})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_login:
               if(checkNull()){return;}
                    List<LoginInfoBean> loginList = DataSupport.where("username = ? and usertype = ?", etUsername.getText().toString(),
                            "开发人员".equals(etUsertype.getText().toString()) ? "0" : "1").find(LoginInfoBean.class);
                        if(loginList.isEmpty()){
                           ...
                           ToastUtil.showToastShort("用户名错误");
                            ...
                            SharePreferenceUtil.setIntSP("currentusertype", "开发人员".equals(etUsertype.getText().toString()) ? 0 : 1);
                            SharePreferenceUtil.setStringSP("currentusername",etUsername.getText().toString());
                            SharePreferenceUtil.setStringSP("currentpsaaword",etPassword.getText().toString());
                            //开发人员  高级界面
                            Intent i=new Intent(this,
                                    "开发人员".equals(etUsertype.getText().toString()) ?
                                            BleSuperActivity.class : BleAddressingTypeActivity.class);
                            startActivity(i);
                            finish();
                        }else{
            ...
            case R.id.iv_username_arrow:
                if(listPopupWindow.isShowing()){
                    ...
                    list.clear();
                    for (LoginInfoBean bean: logininfolist) {
